...
const double RFEAWH = gridsize*5*3;
const double beamsize = 5*gridsize;
...
void simu( int argc, char **argv )
{
    ...
    while (E<200)
    {
        ...
        current[0][i]=pdb.get_statistics().bound_collisions(6);
        current[1][i]=E;
        i++;
        cout<<E<<endl;
        E+=clip(abs((E-Vdis)/4),0.1,20);
    }
    for(int j=0;j<i;j++){
        cout<<current[1][j]<<'\t'<<current[0][j]<<endl;
    }
}
